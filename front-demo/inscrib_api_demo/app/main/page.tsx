...
import { Home, Settings, User, Bell, Upload, Brain, Mic } from 'lucide-react';
...
export default function Main() {
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/report-generation/upload`, {
        method: 'POST',
        headers: {
  ...
  useEffect(() => {
    if (loading) {
      const txts = ["Ironmind Thinking...", "Ironmind Analyzing...", "Ironmind Generating...", "Ironmind Writing..."];
      let i = 0;
  ...
  const generatReport = async () => {
    ...
    //加载数据
    let tplUrl = reportDatas.TemplatePath.replace('http://localhost:3001','http://frontend:3001');
    const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/report-generation/generate?UploadFile=${encodeURIComponent(reportDatas.UploadFile)}&TemplatePath=${encodeURIComponent(tplUrl)}`, {
      method: 'GET',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
  ...
  const changeTemplate = async (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 mt-16 p-6 bg-gray-50">
        ...
        {isShowReport ? (
          ...
          >
              <option value="http://localhost:3001/01.json1">Template1</option>
        ...
        {loading ? (
            ...
            <span className="text-gray-700 mt-2" id='LoadingText'>{loadingText}</span>
